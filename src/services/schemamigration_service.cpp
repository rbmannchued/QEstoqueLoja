...
SchemaMigration_service::Resultado SchemaMigration_service::update() {
    ...
    qDebug() << "ultima versao schema " <<  dbSchemaLastVersion;

    qDebug() << "versao atual schema" << dbSchemaVersion;
    qDebug() << db.tables();

    db.close();
    return {true, SchemaErro::Nenhum, "erro ao iniciar banco de dados", dbSchemaVersion};
